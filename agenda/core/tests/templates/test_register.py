...
class Register_OK_Test(TestCase):
    ...
    def test_register_post_data(self):
        ...
        self.assertTrue(Agenda.objects.filter(nome_completo='casemiro test').exists())
    ...
    def test_register_invalid_phone(self):
        ...
        self.assertFalse(Agenda.objects.filter(nome_completo='casemiro test').exists())

    def test_name_with_special_characters(self):
        self.client.login(email='admin@fatec.sp.gov.br', password='fatec', username='admin')    
        data= {
            'nome_completo' : 'casemiro test 1 @#$%',
            'telefone' : '19999999902',
            'email' : 'test@fatec.sp.gov.br',
            'observacao' : 'teste'
        }
        ...
        self.assertFalse(Agenda.objects.filter(nome_completo='casemiro test 1 @#$%').exists())
